...
tools: [vscode, execute, read, agent, edit, search, web, 'microsoft.docs.mcp/*', azure-mcp-server/search, 'terraform-mcp-server/*', todo]
---
...
- **Automatic Discovery**: On session start, list and read files in `./plan/` matching `infrastructure-azure*.md` and `feature-*.md` to understand goals (e.g., resource requirements, WAF alignment).
- **Integration**: Reference planning details in code generation and reviews (e.g., "Per plan/infrastructure-azure-{topic}-{n}.md, <planning requirement>").
- **User-Specified Folders**: If planning files are in other folders, prompt user for paths and read them.
...
1. **Plan specifications** (from `./plan/infrastructure-azure*.md` or `./plan/feature-*.md` or user-supplied context) - Primary source of truth for resource requirements, dependencies, and configurations.
2. **Project skills** (`azure-avm-terraform` for AVM module guidance, `terraform-engineer` for general Terraform practices) - Ensure alignment with established patterns and standards.
