...
| `docs/openclaw-operations.md` | Bootstrap steps, token management, and upgrade procedures (AKS) |
| `docs/secrets-inventory.md` | Secret names and Key Vault references |

Key facts:
- OpenClaw state is on an Azure Disk PVC (`managed-csi-premium`, 10 Gi, ReadWriteOnce) mounted at `/home/node/.openclaw` in the pod (provisioned by the Helm chart via the `managed-csi-premium` StorageClass)
...
3. Apply config changes — `openclaw config set` and `openclaw configure` write to the local `~/.openclaw` directory, **not** the remote gateway pod:
   ...
   - Never edit `openclaw.json` directly in the pod; never edit the ConfigMap generated by ArgoCD (changes will be overwritten on next sync).
...
Follow `docs/openclaw-operations.md` for the full flow. Key steps:
...
- **Bulk updates via Helm/ArgoCD** — for bulk config updates, update `workloads/<env>/openclaw/values.yaml` in Git and merge; ArgoCD syncs automatically. For an immediate single key: `kubectl exec -n openclaw deployment/openclaw -- node /app/openclaw.mjs config set <key> <value>`. Never edit `openclaw.json` directly in the pod.
...
- **`kubectl exec` sessions are not rate limited.**
