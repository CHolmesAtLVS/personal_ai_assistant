...
> **Note:** `azure-ai-api-key` is seeded from a Terraform variable (`TF_VAR_azure_ai_api_key`) on first apply and is therefore written to Terraform remote state (sensitive output, encrypted at rest). Subsequent applies do not overwrite it due to `lifecycle { ignore_changes = [value] }`. The `openclaw-gateway-token` is generated by Terraform (`random_id`) and is also present in state.
