...
- **SEC-001**: The gateway `auth.token` value in `openclaw.json` must be sourced from Key Vault at deploy time and written to a temp file only during the workflow run. The temp file must be deleted immediately after upload.
- **SEC-002**: The committed config template must contain no placeholder secrets or sample token values — only a clearly named environment variable reference (e.g., `${GATEWAY_TOKEN}`) that is substituted by the workflow.
...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | ---- |
| TASK-000 | Re-seed `openclaw.json` on dev Azure Files. The file must contain at minimum: `gateway.mode="local"`, `gateway.port=18789`, `gateway.bind="lan"`, `gateway.auth.mode="token"`, and `gateway.controlUi.allowedOrigins=["https://paa-dev-app.wittydesert-e5a4134e.eastus.azurecontainerapps.io"]`. Do NOT include `gateway.auth.token` — the KV-injected `OPENCLAW_GATEWAY_TOKEN` env var supplies it. Use `az storage file upload` with the storage account key. Follow the tech runbook section 1.3 procedure. Confirm the Container App revision reaches `Running / Healthy` state after re-seed. Root cause: file on share had `"bind": "all"` (invalid). Fixed to `"bind": "lan"`. Revision `0000013` reached `Running / Healthy` after upload + `az containerapp update --min-replicas 1` to force activation (with `min_replicas=0`, the control plane needed a traffic trigger; min-replicas restored to 0 after verification). | ✅        | 2026-03-30 |
